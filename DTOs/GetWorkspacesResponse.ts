...
export class GetWorkspacesResponse {
    public readonly workspaces: WorkspaceResponse[];

    constructor(workspaces: WorkspaceWithRelations[]) {
        this.workspaces = workspaces.map(workspace => new WorkspaceResponse(workspace));
    }
}   